...
Prerequisites
1. 
Oddworld: New 'n' Tasty (PC Version).
2. 
MelonLoader (Version 0.5.7 is REQUIRED).
3. 
A Screen Reader (Tested with NVDA).
Installation
1. 
Download the MelonLoader Installer.
2. 
Run the installer.
• 
...
Click "Install".
3. 
Run the game once to let MelonLoader create the necessary folders, then close it.
4. 
Copy the compiled OddworldAccess.dll into the Mods folder in your game directory.
5. 
Important: Copy Tolk.dll and nvdaControllerClient32.dll directly into the game's root folder (where NNT.exe is located), NOT into the Mods folder. This is required for speech output.
Controls & Features
General
